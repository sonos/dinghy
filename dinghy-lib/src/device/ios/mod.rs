...
impl IosDevice {
    ...
    fn make_app(&self, project: &Project, run_env: &RunEnv, runnable: &Runnable) -> Result<BuildBundle> {
        ...
        let build_bundle = make_ios_app(project, run_env, runnable, &app_id)?;

        xcode::sign_app(&build_bundle, &signing)?;
        Ok(build_bundle)
    }

    fn install_app(&self, project: &Project, run_env: &RunEnv, runnable: &Runnable) -> Result<BuildBundle> {
        let build_bundle = self.make_app(project, run_env, runnable)?;
...
impl IosSimDevice {
    fn install_app(&self, project: &Project, run_env: &RunEnv, runnable: &Runnable) -> Result<BuildBundle> {
        let build_bundle = IosSimDevice::make_app(project, run_env, runnable)?;
    ...
    fn make_app(project: &Project, run_env: &RunEnv, runnable: &Runnable) -> Result<BuildBundle> {
        make_ios_app(project, run_env, runnable, "Dinghy")
    }
}
...
fn mk_result(rv: i32) -> Result<()> {
    ...
    } else if rv as u32 == 0xe8000022 {
        Err(
            "error: 0xe8000022, kAMDInvalidServiceError. (This one is relatively hard to diagnose... best option, go back to the XCode project and have run the dummy app on your phone to make XCode fix everything for us).",
        )?
    } else if rv != 0 {
...
fn make_ios_app(project: &Project, run_env: &RunEnv, runnable: &Runnable, app_id:&str) -> Result<BuildBundle> {
    let build_bundle = make_remote_app_with_name(project, run_env, runnable, Some("Dinghy.app"))?;
