...
use { BuildBundle, Runnable, RunEnv };
...
fn make_remote_app(project: &Project, run_env: &RunEnv, runnable: &Runnable) -> Result<BuildBundle> {
    make_remote_app_with_name(project, run_env, runnable, None)
}

fn make_remote_app_with_name(project: &Project, run_env:&RunEnv, runnable: &Runnable, bundle_name: Option<&str>) -> Result<BuildBundle> {
    ...
    for src_lib_path in &run_env.dynamic_libraries {
    ...
    debug!("Copying src {} to bundle {}", runnable.src.display(), bundle_path.display());
    project.rec_copy_excl(&runnable.src, &bundle_path, false, &[runnable.src.join("target")])?;
